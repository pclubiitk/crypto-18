...
t0 = Positive_RA_Alt_Thresh[0];    Positive_RA_Alt_Thresh[1] = 500;
t1 = Positive_RA_Alt_Thresh[1];    Positive_RA_Alt_Thresh[2] = 640;
t2 = Positive_RA_Alt_Thresh[2];    Positive_RA_Alt_Thresh[3] = 740;
t3 = Positive_RA_Alt_Thresh[3];location += (location == 0)*  2  ;
 location += (  t0 < Other_Tracked_Alt   )*(   9   -  2  )*( !( location -   2  ));
;
return ; 
}
...
bool Non_Crossing_Biased_Climb()
{
 ...
 result = !(Own_Below_Threat()) || ((Own_Below_Threat()) && (!(Down_Separation > ALIM())));
    location += (  Climb_Inhibit < Own_Tracked_Alt   ) * (  15   -   12  ) * ( ! ( location -   12  ));
;
}
    else
    {
 result = Own_Above_Threat() && (Cur_Vertical_Sep >= 300) && (Up_Separation >= ALIM());
    ...
    location += (  Other_RAC < Own_Tracked_Alt_Rate   ) * (  18   -   15  ) * ( ! ( location -   15  ));
;
return result;
return 0 ; 
}
...
bool Non_Crossing_Biased_Descend()
{
 ...
 result = Own_Below_Threat() && (Cur_Vertical_Sep >= 300) && (Down_Separation >= ALIM());
    location += (  Other_RAC == t8   ) * (  21   -   18  ) * ( ! ( location -   18  ));
;
}
    else
    {
 result = !(Own_Above_Threat()) || ((Own_Above_Threat()) && (Up_Separation >= ALIM()));
    ...
    location += (  Down_Separation == t1   ) * (  29   -   21  ) * ( ! ( location -   21  ));
;
return result;
return 0 ; 
}
...
bool Own_Above_Threat()
{
    location += (  sys_nerr < Own_Tracked_Alt_Rate   ) * (  33   -   29  ) * ( ! ( location -   29  ));
;
...
alt_sep = 0;
;} // end 'if' part
    location += (  Alt_Layer_Value == alt_sep   ) * (  35   -   33  ) * ( ! ( location -   33  ));
;
}

    location += (  Climb_Inhibit == alt_sep  ) *(-1 -   35  )*( !(location -   35  ));
;
return alt_sep;
return 0 ; 
}
...
typedef int _t __attribute__ ((__mode__ (__word__)));
